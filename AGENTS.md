...
- Write the root `README.md`, subdirectory README files, and documents under `docs/` primarily in Japanese.
- Keep code identifiers, API paths, commands, configuration keys, and canonical technical terms in English when that is clearer.
- Add English text only when it materially improves technical clarity or interoperability.
...
StockLens AI uses Specification-Driven Development (SDD). Approved specifications under `specs/` are the feature-level source of truth. Cross-cutting product, architecture, security, and database decisions remain under `AGENTS.md`, `docs/`, and ADRs.

### Artifact Order

For new features and behavior changes, use this order:

1. Create or update `spec.md`.
2. Resolve material open questions and obtain user approval.
3. Create or update `technical-plan.md`.
4. Break the work into `tasks.md` items that reference Requirement IDs.
5. Implement the approved scope.
6. Verify every Acceptance Criterion with tests or documented evidence.
7. Record results and remaining gaps in `verification.md` and `specs/traceability.md`.

Exploratory, read-only investigation is allowed before approval. Production behavior, API contracts, database schema, security policy, or external side effects must not be changed while a material decision is unresolved.

### Requirement IDs and Traceability

- Functional requirements use `<FEATURE>-FR-###`.
- Security requirements use `<FEATURE>-SEC-###`.
- Acceptance criteria use `<FEATURE>-AC-###`.
- Requirement IDs are stable and must not be silently reused for different behavior.
- Code and test names should reference Requirement IDs when practical.
- Every implemented Requirement must map to implementation evidence and verification evidence in `specs/traceability.md`.

### Status and Deviations

- Spec status is one of `Draft`, `Backfilled`, `Approved`, `Implementing`, `Implemented`, `Verified`, or `Superseded`.
- Backfilled specifications describe existing behavior but are not automatically approved.
- Any mismatch among Spec, code, tests, documentation, or security rules must be recorded in `specs/deviations.md`.
- Do not hide, silently fix, downgrade, or omit a deviation discovered during backfill or implementation.
- A deviation that changes security, public behavior, data integrity, architecture, or scope requires an explicit user decision before remediation.
- Verification may be `Passed`, `Partial`, `Failed`, or `Blocked`; mocked tests alone cannot justify `Passed` for integration behavior.

### Definition of Done

A feature is `Verified` only when:

- Its Spec is approved and has no unresolved material questions.
- Technical Plan and Tasks match the implemented scope.
- API, database, security, and operational documentation are updated where applicable.
- Acceptance Criteria have passing automated tests or explicit, reproducible verification evidence.
- Lint, typecheck, relevant tests, and build pass.
- Known deviations and residual risks are visible and have an owner or decision.
...
3. Read the relevant feature Spec and traceability entries.
4. Inspect existing code before editing.
5. Provide a short implementation plan and identify likely affected files.
6. Record unresolved material decisions instead of assuming them.
7. Update the Spec, Technical Plan, and Tasks before changing behavior.
8. Make scoped changes only.
9. Run relevant lint, typecheck, tests, and Acceptance Criteria verification.
10. Update Verification, Traceability, and Deviations.
