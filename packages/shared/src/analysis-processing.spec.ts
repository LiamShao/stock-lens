import {
  ...
  parseAnalysisViewsIdempotencyKey,
...
describe('structured extraction job contract', () => {
  it('EXTRACT-FR-001 fixes stable job names and versioned idempotency keys', () => {
    ...
    expect(ANALYSIS_GENERATE_VIEWS_JOB_NAME).toBe('generate-analysis-views');
    expect(
  ...
  it('VIEW-FR-001 fixes the analysis views execution identity', () => {
    const input = {
      analysisId,
    ...
    expect(
      parseAnalysisViewsIdempotencyKey(
        createAnalysisViewsIdempotencyKey(input),
      ),
    ).toEqual(input);
  });
