...
- 単一 Object の `GetObject` と `application/pdf` Response Content Type に限定した Presigned GET URL を発行
...
- `@aws-sdk/s3-request-presigner`: S3 Client Credential を公開せず短命 PUT/GET URL を署名するために必要
...
- API は Active Owner/Analysis/Finalized Document と Configured Bucket を確認します。
- Adapter の `HeadObject` で存在を確認してから、同じ Key に最大 300 秒の `GetObject` URL を発行します。
- API Response は URL/Expiry だけとし、`Cache-Control: no-store` を返します。
- Missing Object、Provider Failure、不正な Expiry は Storage Detail を含まない Stable `503` に変換します。
