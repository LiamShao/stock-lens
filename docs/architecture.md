...
Phase 5 View Generation は Phase 4 Publish Transaction で Exact Source/Prompt/Schema/Runtime Identity を固定した Durable `GENERATE_VIEWS` Execution を作成します。Worker は `READY_FOR_VIEW_GENERATION` の Active Parent、Finding-linked Evidence、Original Document/Page/Chunk/Excerpt、Financial Metrics、Prompt/Input Identity を再解決し、Bounded Repair/Retry 後に三 View JSONB、`COMPLETED`、`completedAt`、Execution/Attempt Success を同一 Transaction で保存します。Redis 投入失敗は Pending Dispatcher、Automatic Retry Exhaustion は既存 Operator CLI が同一 Execution で回復します。
...
Evidence PDF の Read Presign は API が Active Owner/Analysis/Finalized Document と Object `HEAD` を確認し、単一 Object の `GetObject` を最大 300 秒で署名します。Response は URL/Expiry だけで `no-store` とし、Missing/Provider Failure は Storage Coordinate を除去した Stable `503` にします。Web は Citation Drawer の明示操作時だけ URL を取得し、`no-store` / Credential Omit / Redirect Reject で PDF Bytes に交換します。URL を State/Cache/DOM/Log に残さず、PDF.js の Version-matched Worker と Canvas-only Page Renderer で Evidence Page を開きます。
